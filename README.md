...
- **Backend** — FastAPI (async): роутеры `auth`, `user`, `events`, `admin`, `organizations`, `olympiads`, `docs`, `imports`, `health`; слой доступа к данным на async SQLAlchemy; Pydantic-схемы.
...
- **Celery** — очередь `heavy` для CPU-нагруженной обработки: OCR PDF-таблиц и RSOSH-импорт (пайплайн выполняется в subprocess).
...
- объекты уровня «событие», «профиль», «документ» и списки кэшируются в Redis с инвалидацией по версиям;
...
- тесты: полный набор 52 passed (`docker compose --profile testing run --rm test`);
