...
- каталог олимпиад и страница конкретной олимпиады (список — карточками, 3–4 в ряд);
...
- главный экран SPA переведён на каталог олимпиад (карточки 3–4 в ряд), навигация без «Мои события» и дублирующего пункта «Импорты», «Админ» подсвечивается один раз;
...
- тесты: полный набор 36 passed (`docker compose --profile testing run --rm test`);
