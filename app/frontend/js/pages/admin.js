 ...
 * Страница для API-маршрутов /admin/*, у которых раньше не было UI.
...
function renderAdmin() {
    ...
    if (!store.isAdmin()) {
        ...
        <div class="max-w-narrow mx-auto py-24 md:py-32 text-center">
            <div class="mx-auto w-16 h-16 rounded bg-tint flex items-center justify-center mb-10" aria-hidden="true">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#064789" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="4" y="11" width="16" height="10" rx="1"></rect>
                    <path d="M8 11V7a4 4 0 0 1 8 0v4"></path>
                </svg>
            </div>
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight">Доступ ограничен</h1>
            <p class="mt-6 text-lg text-ink/60 leading-relaxed max-w-md mx-auto">Этот раздел доступен только администраторам платформы.</p>
    ...
    <section class="pt-4 pb-12">
        ...
        <p class="mt-6 text-lg text-ink/60 leading-relaxed max-w-2xl">Управление пользователями и событиями: роли, блокировки, архив.</p>
    </section>

    <!-- Сегментированные вкладки: активная выделяется фоном и тенью, без границ -->
    <div class="flex sm:inline-flex flex-wrap gap-2 bg-tint rounded p-2 mb-12" role="tablist" aria-label="Разделы администрирования">
        <button id="admin-tab-users" role="tab" aria-selected="true" class="flex-1 sm:flex-none sm:min-w-[10rem] px-6 py-3 rounded text-sm font-semibold bg-white text-ink shadow-elev-1 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">Пользователи</button>
        <button id="admin-tab-events" role="tab" aria-selected="false" class="flex-1 sm:flex-none sm:min-w-[10rem] px-6 py-3 rounded text-sm font-semibold text-ink/55 hover:text-ink transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary">События</button>
    </div>
...
function selectAdminTab(tab) {
    ...
    const base = 'flex-1 sm:flex-none sm:min-w-[10rem] px-6 py-3 rounded text-sm font-semibold transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-primary';
    usersBtn.className = tab === 'users' ? `${base} bg-white text-ink shadow-elev-1` : `${base} text-ink/55 hover:text-ink`;
    eventsBtn.className = tab === 'events' ? `${base} bg-white text-ink shadow-elev-1` : `${base} text-ink/55 hover:text-ink`;
...
function usersListHtml(users) {
    if (!users.length) return `<p class="py-24 text-center text-lg text-ink/50">Пользователи не найдены</p>`;

    const selfId = store.user ? store.user.id : null;

    return `<p class="text-sm text-ink/45 mb-6">Всего: ${users.length}</p>
    <div class="space-y-6">
        ${users.map(u => {
            ...
            const roleBadge = u.role === 'ADMIN'
                ...
                : `<span class="${UI.badge} ${UI.badgeNeutral}">Пользователь</span>`;
            ...
            <div class="${UI.card} px-8 py-7 flex flex-col xl:flex-row xl:items-center gap-6">
                <div class="flex items-center gap-5 flex-1 min-w-0">
                    <div class="w-12 h-12 rounded bg-tint text-primary font-bold flex items-center justify-center shrink-0" aria-hidden="true">${escHtml(initials)}</div>
                    <div class="min-w-0">
                        <p class="font-bold text-ink truncate">${escHtml(u.name)} ${escHtml(u.surname)}${isSelf ? ' <span class="text-ink/40 font-medium">(вы)</span>' : ''}</p>
                        <p class="text-sm text-ink/50 truncate">${escHtml(u.email)}</p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2 flex-wrap xl:justify-end shrink-0">
                    ...
                    ${u.isActive
                        ...
                        : `<button data-act="unban" data-id="${escAttr(u.id)}" class="${UI.btn} ${UI.btnSuccess} ${UI.btnSmall}">Разблокировать</button>`}
                </div>
            </div>`;
        }).join('')}
    </div>`;
...
function eventsListHtml(events) {
    if (!events.length) return `<p class="py-24 text-center text-lg text-ink/50">Событий пока нет</p>`;

    return `<p class="text-sm text-ink/45 mb-6">Всего: ${events.length}</p>
    <div class="space-y-6">
        ${events.map(ev => {
            ...
            <div class="${UI.card} px-8 py-7 flex flex-col lg:flex-row lg:items-center gap-6">
                <div class="flex-1 min-w-0">
                    ...
                    <p class="mt-2 text-sm text-ink/50">Создано ${formatDate(ev.createdAt)}</p>
                </div>
                <div class="flex items-center gap-3 flex-wrap shrink-0">
                    ${statusBadge}
