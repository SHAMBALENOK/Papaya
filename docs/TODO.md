...
# ✅ Выполнено — ветка `patch-0.7`

- **Удалена legacy-подсистема «события»**: таблица/роутер/схемы/модель/кэш-слой/фронтенд-страницы удалены, миграция `0005` дропает таблицу `events` на существующих базах (в свежих её уже нет); PyTorch и ddgs убраны из зависимостей.
- Главный экран SPA — каталог олимпиад (карточки 3–4 в ряд); из навигации убраны «Мои события» и дублирующий пункт «Импорты»; «Админ» подсвечивается один раз.
- Доменные сущности **Organizations / Olympiads / Docs** (связи через JSONB-массивы, без преждевременных таблиц) + миграция `0004`.
- REST `/api/v1/organizations`, `/api/v1/olympiads`, `/api/v1/docs` (CRUD, object-level доступ орг-админа, версионный Redis-кэш).
- **RSOSH-импорт**: `app/rsosh` (извлечение PDF/XLSX → нормализация → сопоставление → статусная машина) + `/api/v1/imports` (старт 202 / preview / confirm / reject / список). Персист только по явному confirm, повторный confirm идемпотентен, дубли объединяются.
- Фронтенд: страницы организаций, олимпиад, документов, предпросмотра импорта, панель администратора (`#/admin/imports`).
- Тесты: `docker compose --profile testing run --rm test` → 36 passed.
- Миграции ролей: `EDITOR` → `ORGANIZATION_ADMIN` (миграция `0003`).
...
**Статус:** реализована в ветке `patch-0.8` (JSONB-поле `schedule`, без отдельных таблиц).
...
## Сделать
- [X] Несколько этапов у одной олимпиады.
- [X] Отдельные даты для каждого этапа.
- [X] Дедлайны.
- [X] Статус каждого этапа.
- [X] Автоматическое вычисление текущего статуса олимпиады.
...
> Как устроено: `schedule` — JSONB в `olympiads.schedule` (миграция `0006`),
> схема `OlympiadSchedule`/`OlympiadStage` в `app/schemas/olympiads.py`,
> вычисление статусов/дедлайнов в `app/core/schedule.py`. Статусы не сохраняются —
> enrich выполняется при формировании ответа. См. детали в `docs/responses.md`.
